...
def evaluate_classifier(trainer: Trainer, tokenized_test) -> dict:
    ...
    f1_values = f1_score(labels, preds, average=None, zero_division=0)
    precision_values = precision_score(labels, preds, average=None, zero_division=0)
    recall_values = recall_score(labels, preds, average=None, zero_division=0)
    ...
    for i in range(len(f1_values)):
        ...
        per_class_f1[label_name] = float(f1_values[i])
        per_class_precision[label_name] = float(precision_values[i])
        per_class_recall[label_name] = float(recall_values[i])
...
def main() -> None:
    ...
    if os.environ.get("DATA_PATH") is not None:
        training_args = make_training_args(output_dir, lr=1e-4, epochs=8, batch_size=8, seed=42)
    else:
    ...
    if os.environ.get("DATA_PATH") is None:
        ...
        try:
            ...
            print(f"\nPushed to https://huggingface.co/HadeelBH/{repo_id}")
